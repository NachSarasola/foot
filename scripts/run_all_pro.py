...
for t in teams:
    ...
    kpis[t]={'shots':int(len(sub)),'goals':int(sub.get('is_goal',0).sum()),'xg':float(round(sub.get('xg',0).sum(),2))}

# PPDA
def ppda(df, team):
    req = {'is_pass','is_def_action','y'}
    if not req.issubset(df.columns): return None
...
def draw_shot_map_pro(shots_df, teams, meta, out_path):
    ...
    for t in teams:
        ...
        pitch.scatter(sub['x'], sub['y'], s=sizes, color=team_color[t], edgecolors='#ffffff', linewidth=0.8, alpha=0.90, label=t, ax=ax, zorder=4)
...
def draw_xg_race_pro(shots_df, teams, meta, out_path):
    ...
    series={}
    for t in teams:
        by_min=(df[df['team']==t].groupby('minute')['xg_val'].sum().reindex(minutes, fill_value=0).cumsum())
        series[t]=by_min
    ...
    ax.fill_between(minutes, series[teams[0]], series[teams[1]], where=(lead>=0), interpolate=True, color=COLORS['cyan'], alpha=0.08, zorder=0)
    ax.fill_between(minutes, series[teams[0]], series[teams[1]], where=(lead<0),  interpolate=True, color=COLORS['blue'], alpha=0.08, zorder=0)

    annotate_goals_on_xg(ax, df, teams[0]); annotate_goals_on_xg(ax, df, teams[1])
    ax.axvline(45, color='white', alpha=0.15, lw=1); ax.text(45, ax.get_ylim()[1]*0.05, "HT", color=COLORS['fog'], ha='center', va='bottom', fontsize=9)

    ax.set_xlim(0, minutes[-1]); ax.set_xlabel('Minuto'); ax.set_ylabel('xG acumulado')
...
# ====== HTML ======
report_path = REPORT/'river_libertad_report.html'
render_html_report_pro(meta, teams, kpis, ppda_vals, shotmap_path, xgrace_path, passnet_path, BRAND/'ush_logo_dark.svg', report_path)
